	...
	*** Retrieve scale name and scale levels from user input
	...
	foreach scale of local namelist { //iterate over scales
		...
		*** Retrieve levels from items
		...
		if "`labs'" ~= "" {
			*** Compare LABELs with user input
			...
			if "`s(differences)'" == "" {
				di in y "LABELS: user input matches label information."
			}
			else {
				di in y "LABELS: label values are DIFFERENT from user input. " ///
		...
		*** Compare ITEM CATS with user input
		...
		if "`s(differences)'" == "" {
			di in y "DATA: user input matches categories in items."
		}
		else {
			di in y "DATA: item categories are DIFFERENT from user input. " ///
			_n "    Difference are: `s(differences)'" /// 
			_n "    Using user input"
		}
		...
		*** Creates the file with item cummulative distributions		
		local counter = 1
		qui foreach item of local scale_items { // iterate over items of a scale
