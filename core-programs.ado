...
program define ifeats, rclass
	...
	syntax namelist, Nobs(numlist) /// 
	       [NWitems(integer 3) NTitems(integer 12) propmiss(real 0) nwavemiss(integer 1) /// 
	...
	**** Todo. 
		...
		* di "`nTotalItems'"
			ifeatsCore `lobs' 1 `cols' `rows' `nwitems' 1 `propmiss' `mblock' `simcorr' `simmarginals' `corrmatrix' `marginals' "`namelist'" "`nTotalItems'" `nwavemiss'
			local ++cols
		}
	}
	mat list simmat
...
*/
	...
	*** Random missing pattern
	...
	if `mblock' ~= 1 {
		qui foreach scale of local namelist {
			...
			noi di in y "Complete cases in scale `scale': `=round(`r(mean)', .001) * 100'%"
		}
	}
	...
	*** Block random missing pattern
		...
		*** and create a set of new variables that contain the periods for which obs is missing
		...
		mata: myperiod = rdiscrete(ssize, strtoreal("`nwavemiss'"), timePoints) // sample periods
		/* st_local("missitem", strofreal(myperiod)) */
		forval i = 1/`nwavemiss' {
